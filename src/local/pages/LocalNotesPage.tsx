...
// Local NotesPage. Structurally follows the original Catavyn NotesPage (grid/
// list, category + tag filters, search, sub-notes, note lock) but is wired to
// the local hooks and drops collaboration. NoteEditor/NoteCard are reused as-is.

function EditorWithSubNotes(
  props: React.ComponentProps<typeof NoteEditor> & { noteId: string },
) {
...
export default function LocalNotesPage({
  ...
  const activeTagFilter = tags.find(t => t.id === tagFilter) ?? null

  // When the shell requests a specific note (from Home/Pinned/Tags), open it
  // once the notes list has loaded. Locked notes go through the PIN modal.
  useEffect(() => {
    if (!openNoteId || loading || editorOpen) return
  ...
  async function handleCategoryChange(catId: string | null) {
    if (editingNoteId) await updateNote(editingNoteId, { category_id: catId })
    else setPendingCategoryId(catId)
  }
  ...
  async function handleTagRemove(tagId: string) {
    if (editingNoteId) await detachTag(editingNoteId, tagId)
    else setPendingTagIds(prev => prev.filter(id => id !== tagId))
  }

  async function handleTagCreate(name: string) {
    const created = await createTag({ name })
    if (created) {
      if (editingNoteId) await attachTag(editingNoteId, created)
      else setPendingTagIds(prev => [...prev, created.id])
    }
    return created
  }
    ...
    <>
      {editorOpen ? (
        ...
        <div className="animate-fade-up p-4 sm:p-6 max-w-5xl mx-auto">
          ...
          <SearchBar placeholder="Search notes…" onSearch={setSearchQuery} className="mb-4" />
          ...
          {activeTagFilter && (
            <div className="flex items-center gap-2 mb-4">
              ...
              >
                ...
                <button type="button" onClick={() => setTagFilter(null)} aria-label="Clear tag filter" className="hover:opacity-75 leading-none">
                  <X size={11} />
                </button>
              </span>
            </div>
          ...
          {loading ? (
            ...
            <div className="flex flex-col gap-2">
              {filteredNotes.map(note => (
                <NoteListRow key={note.id} note={note} onClick={() => openEdit(note)} onDelete={() => handleDelete(note)} />
              ))}
            </div>
          )}
        </div>
...
function NoteListRow({ note, onClick, onDelete }: { note: Note; onClick: () => void; onDelete: () => void }) {
  ...
  return (
    <div className="group flex items-center gap-2">
      ...
      >
        <div className="text-text-primary font-medium text-sm truncate flex-1 min-w-0">{note.title || 'Untitled'}</div>
...
function UndoToast({ title, onUndo }: { title: string; onUndo: () => void }) {
  return (
    <div className="fixed bottom-24 md:bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-4 bg-text-primary text-bg-page rounded-xl px-4 py-3 shadow-lg text-sm whitespace-nowrap">
      ...
      <button onClick={onUndo} className="font-semibold text-accent-gold hover:opacity-80 transition-opacity shrink-0">
        Undo
      </button>
    </div>
  )
