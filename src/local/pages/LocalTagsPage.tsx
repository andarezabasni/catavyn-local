...
// Local Tags page. Same layout as the original; onTagClick lets the shell jump
// to the notes view filtered by that tag.
export default function LocalTagsPage({ onTagClick }: { onTagClick?: (tagId: string) => void }) {
  const { tags, noteTagsMap, loading, createTag, deleteTag } = useTags()

  const tagNoteCount = Object.values(noteTagsMap).reduce<Record<string, number>>((acc, noteTags) => {
    for (const tag of noteTags) acc[tag.id] = (acc[tag.id] ?? 0) + 1
    return acc
  }, {})
    ...
    <div className="animate-fade-up p-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-text-muted text-xs mt-0.5">{tags.length} {tags.length === 1 ? 'tag' : 'tags'}</p>
        </div>
        {!showForm && (
          <button onClick={() => setShowForm(true)} className="flex items-center gap-1.5 text-sm text-accent-gold hover:opacity-75 font-medium transition-opacity">
            <Plus size={15} />
            New tag
          </button>
        )}
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="mb-6 bg-bg-card rounded-xl border border-border p-4 flex flex-wrap gap-3 items-center">
          <input
            type="text"
          ...
          <button type="button" onClick={() => { setShowForm(false); setTagName('') }} className="rounded-lg px-3 py-1.5 text-sm text-text-muted hover:text-text-secondary transition-colors">
            Cancel
          </button>
        </form>
      )}
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-dashed border-border p-12 text-center">
          ...
          <button onClick={() => setShowForm(true)} className="text-sm text-accent-gold font-medium hover:opacity-75 transition-opacity">
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-text-primary font-medium text-sm truncate">{tag.name}</span>
                </div>
                <div className="text-text-muted text-xs">{count} {count === 1 ? 'note' : 'notes'}</div>
                <button
                  type="button"
