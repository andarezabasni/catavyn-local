...
export default function LocalPinnedPage({ onOpenNote }: { onOpenNote?: (id: string) => void }) {
  const { notes, loading, togglePin, deleteNote } = useNotes({ rootOnly: true })
    ...
    <div className="animate-fade-up p-6 max-w-5xl mx-auto">
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-dashed border-border p-12 text-center">
          ...
          <p className="text-text-muted text-xs mt-1">Pin a note from the editor or note card to see it here.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
